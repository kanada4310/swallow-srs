...
# usage: python build_workflow.py <words.json> <out.js> <wf_name>
...
7. 数字・冠詞・代名詞・間投詞・固有名詞など、定型コロケーションが乏しい語は、3つの【異なる】自然な定番フレーズ／使い方で代用してよい。
   例: three→"three times","for three days","three of us" / the→"the same","the best","all the way"。
...
'''
js = tpl.replace('%NAME%', name).replace('%DATA%', DATA)
