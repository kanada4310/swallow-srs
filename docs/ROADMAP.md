...
- [ ] 成長段階導出（`stability` → 種/芽/苗/成株/開花・結実）
- [ ] 世話状態導出（`due` 超過度 → 健やか/乾き気味/しおれ/枯れかけ/枯れ）
- [ ] `lapses`（弱り）・`difficulty`（個性＝育てにくさ）の反映
- [ ] しきい値の実データ検証・調整（FSRS retention 依存）
- [ ] ユニットテスト（段階遷移・境界値）

### 10.2 ハビタット（箱庭／野原）ビュー ★体験の中心
デッキを箱庭/野原として表示。「今日水やりが必要な株」を学習開始導線に。

- [ ] デッキ単位の野原/果樹園/花畑ビュー（株が点在、成長段階・水やり要否を表示）
- [ ] 「今日 水やりが必要 N株」サマリー → ワンタップ学習（＝水やり）
- [ ] オフライン（Dexie card_states）から導出
- [ ] 学習画面に「水やり→成長」演出・成長アニメ

### 10.3 枯れ＆芽吹き直し
放置の帰結を可視化しつつ、SRSの科学を壊さない。

- [ ] 長期放置でしおれ→枯れ（コスメティックのみ、card_states 不変）
- [ ] 久々の復習→FSRS自然リセット（Again→stability低下）で種/芽から育て直し
- [ ] 枯れ株に水やり（=復習）で芽吹き直す導線（永久ロスト禁止＝安全弁）
- [ ] 枯れ株一覧ビュー（水やりで戻せる株の一覧）
