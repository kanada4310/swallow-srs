...
### 6.2 ページ遷移パフォーマンス改善 ★高優先
N+1クエリ解消とページ遷移時の体感速度向上。

- [ ] デッキ一覧のN+1クエリ解消
  - 原因: decks/page.tsx でデッキ毎にカード数・状態を個別クエリ（10デッキ=32クエリ）
  - 修正: JOINまたはバッチクエリで2-3クエリに集約
- [ ] 講師ダッシュボードのN+1クエリ解消
  - 原因: page.tsx で生徒毎に5クエリ（50生徒=252クエリ）
  - 修正: バッチクエリまたはDB関数で集約
- [ ] ミドルウェアのプロフィールチェック最適化
  - 原因: 全リクエストでprofileテーブルをSELECT
  - 修正: セッションクッキーにrole情報を含める or キャッシュ
- [ ] `router.refresh()` を楽観的UI更新に置換
  - 対象: DeckDetailClient.tsx のノート追加・インポート・OCR・例文生成後
- [ ] デッキ一覧・ダッシュボードにローディングスケルトン追加
...
**Phase**: Phase 6.1 学習体験の高速化 動作確認完了
**最終更新**: 2026-02-04
**次のタスク**: Phase 6.2 ページ遷移パフォーマンス改善

### 次回セッションでやること

Phase 6 を以下の優先順で実装：
1. ~~**Phase 6.1**: 学習体験の高速化（カード切り替え遅延解消）~~ ✅ 完了
2. **Phase 6.2**: ページ遷移パフォーマンス改善（N+1クエリ解消）
