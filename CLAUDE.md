...
- **次にやること**:
  1. **Vercel デプロイ＋実機確認**（新デザイン・ミッションカード・コーチマーク・古文単語演習）。スマホ確認は `scripts/device-preview.mjs`（Playwright・テスト生徒に自動ログイン・Git Bash では `MSYS_NO_PATHCONV=1` 必須）
  2. **チュートリアルデッキ「つばめSRSのつかいかた」を講師UIからクラスへ配布**（現在テスト生徒のみ）
  3. **SRS総合評価レポートの改善実装**: 最優先=learn-ahead（学習ステップ待ちの解消）→ 練習モードの記録方式 → 速度→簡単判定の重み → 同期差分化 → 送信キュー消失対策（詳細は docs/progress.md 2026-07-07 メモ）
