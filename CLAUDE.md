...
- **純ロジック** `src/lib/pen-syntax/`（テスト29件＋計測ベンチ7件）: 幾何特徴（`geometry.ts`）・
  $P点群照合の自前実装（`pdollar.ts`）・形の記号判別（`shapes.ts`・丸/山括弧は円弧と2本線の当てはめ比較）・
  文字判別（`letters.ts`・**有限候補への当てはめ**＝上の行なら品詞11種・下の行なら働き8種）・
...
- **機械計測**（種固定・`benchmark.test.ts`）: 群A一発90.6%/救済込み99.2%・群B99.0%・
  働き99.1%・品詞86.6%/97.3%・吸着100%・手のひら誤反応0。**実機ペンの実測は塾長が pen-lab で行う**
