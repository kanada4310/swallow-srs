...
**Phase 7.6 ノートブラウザ強化 + コピー/移動 🔧 作業中**

API・コンポーネントは実装済み。デッキ一覧埋め込みUIの動作確認で問題あり→専用ページ化が必要。

### 次回セッションでやること
1. **ノートブラウザ専用ページ化（最優先）**:
   - `/notes` 専用ページを作成（デッキ一覧の埋め込みから移行）
   - ナビゲーションメニューに「ノート」ボタン追加（デッキの次）
   - 全ノート検索の結果が少ない問題を調査・修正（searchDeckIdsの内容確認）
...
- **全ノートブラウザ検索結果が少ない**: deckId未指定時、searchDeckIdsが限定的。ユーザー所有デッキ+配布デッキの取得ロジック or RLSの問題。要調査
