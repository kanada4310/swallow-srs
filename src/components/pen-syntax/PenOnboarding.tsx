 ...
 * 必須の記号（括弧8種＋働き7種＋波線＝16種）のお手本を1つずつ登録してもらう。
 * 品詞の英字5種は任意（2026-08-31。通常モードでは品詞の書き込みを求めないため）。
 * 波線は 2026-09-02 に必須へ昇格。登録済みの利用者には、足りない波線だけを
 * 追加登録する案内がこの同じ流れで自動的に出る（missingRequired が続きから再開）。
...
function symbolHint(symbol: SymbolId): string {
  ...
  if (symbol === 'wavy') return '単語の下に引く波線＝熟語・慣用表現の印（画面では単語の間で切れないひとつながりに描かれます）'
    ...
    return (
      <div className="mb-3 rounded-card border border-gray-200 bg-white p-4 shadow-card" {...{ [PEN_UI_ATTR]: '' }}>
        ...
        <p className="mb-3 text-sm leading-relaxed text-ink-2">
          ...
          品詞の英字（n・v・a・ad・aux）と下線・＋のお手本は任意です
          （品詞まで書きたい人だけ登録してください。仮・真・強・同の印は手書きせず、
          働きのマスをタッチして付けます）。
  ...
  return (
    ...
    <div className="mb-3 rounded-card border border-sora bg-white p-4 shadow-card" {...{ [PEN_UI_ATTR]: '' }}>
      <p className="mb-1 text-base font-extrabold text-ai">
        {mode === 'redo' ? 'お手本の登録し直し' : 'はじめに: 記号のお手本登録（1回だけ・2〜3分）'}
      </p>
