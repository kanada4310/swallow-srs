/**
 * ノートタイプ「画像マスキング」を作成（Phase 13.4 増分B）。
 *
 * 全ユーザーで使えるよう is_system: true で作成する（Basic/Cloze と同様）。
 * 表示用フィールド 画像表/画像裏 は StudyCard が「マスク領域」(JSON) から合成する。
 *
 * Usage:
 *   node data/create-image-occlusion-notetype.mjs --dry-run
 *   node data/create-image-occlusion-notetype.mjs
 *   node data/create-image-occlusion-notetype.mjs --force   # 既存ならテンプレ/フィールドを更新
 */
import fs from 'fs'
import path from 'path'
import { createClient } from '@supabase/supabase-js'
import { fileURLToPath } from 'url'
import { NOTE_TYPE_NAME, FIELDS, FRONT_TEMPLATE, BACK_TEMPLATE, CSS } from './image-occlusion-template.mjs'
