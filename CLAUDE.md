...
つばめ共通の規程（絶対規程 `constitution.md` ・業務規程 `conduct.md`）は `~/.claude/rules/` にあり、
セッション開始時に自動で読み込まれる。**ここには書き写さない。**
この CLAUDE.md に書くのは、この職場にしかない事情だけ（正本: 統合塾運営エージェント/rules/）。

### クラウドで開いたときの控え（恒久・ADR: cloud-rules-fallback）

**クラウド（claude.ai/code）では、上の共通規程は読み込まれない。**配布先 `~/.claude/` は塾長のパソコンの中にあり、
クラウドは別の計算機で動くため、構造的に届かない（2026-08-24 実測）。
そのため絶対規程の要点だけをここに控えとして置く。ローカルで開いているときは重複した控えになる。

1. **保護者・生徒への送信はしない。**下書きまでで止め、送信は塾長が自分の手で行う。
   例外は、塾長がその場で宛先と全文を確認し「送信してよい」と明示に承認した1回分のみ。
   自動送信・予約送信・事前の包括承認は例外に含まれない
2. **お金の確定操作をしない。**請求の確定、口座振替の送信、購入、振込
3. **元に戻せない操作と、お金が発生しうる操作は、実行前に塾長の承認を得る。**
   削除、上書き、本番への反映、外部への公開、有料サービスの契約、使った分だけ課金される仕組みの利用

判断に迷ったら「破ったら誰かに謝りに行くことになるか」で判定する。なるならここに該当する。
クラウドで作業するときは、可能なら統合塾運営エージェントのリポジトリを同じセッションに追加し、
`rules/constitution.md` と `rules/conduct.md` の全文を読むこと。
...
- **テスト**: `src/lib/syntax-ai/syntax-ai.test.ts` 34件（費用・ゲート・上限装置・直列化・判定JSON・プロンプト・カード描画・進捗互換・入力検査）
...
## スパルタプログラム管理（2026-08-25）✅ 実装済み（**026は本番未適用・承認待ち**）

期間を定めた暗記の取り組み（達成報酬つき）の管理。生徒の毎日のLINE手打ち報告を廃止し、**学習記録そのものを進捗にする**。ADR `20260825-sparta-program-management`。
- **データ**: `sparta_programs`（`026_sparta_programs.sql`）＝生徒×対象ルートデッキ配列×期間×目標カード数×習得基準×状態＋登録時習得数（baseline）。定義だけを持ち、進捗は card_states / review_logs から**集計時に導出**（学習エンジン不変の純表示層）
- **習得判定**: 既存の定着度区分（`wordbook/mastery`）を流用。既定=「定着中以上（実効7日以上）」（1ヶ月内に途中導入したカードは21日に届かないため）・「定着済み」も選択可。進捗= 現在習得 − baseline（対象デッキ/基準の編集で baseline 測り直し）
- **純ロジック** `src/lib/sparta/logic.ts`（テスト23件）: 日本時間・午前4時区切りの日付キー（サーバーUTCでも体感とずれない）・進捗導出・入力検査。サーバー集計 `src/lib/sparta/server.ts`（review-pause の DeckMaps 再利用・service role＋担当生徒チェック＝ADR 20260819 決定6と同型）
- **講師画面** `/students/sparta`（/students/progress から導線）: 登録・編集・中止・削除、一覧に進捗（習得数/目標・進捗率・日々の実施ストリップ・連続・残り日数・今日実施済みか）、終了プログラムに達成サマリー（習得度内訳＝報酬判定の材料。金額計算はしない）。API `/api/teacher/sparta`（GET/POST/PUT/DELETE・`?options=<studentId>` でその生徒の選択可能ルートデッキ）
- **生徒側**: ホームのミッションカード直下に `SpartaCard`（対象デッキ・期限・残り日数・進捗率・今日実施済みか・連続。`/api/sparta/mine`・オフラインや未登録は非表示）
- **催促**: 既存のLINE学習リマインダー（due-cards-summary）が全ルートデッキ対象のためスパルタデッキも自動で乗る（SRS側変更なし・C19後方互換維持）。未着手生徒への催促・期限接近の特別文言は billing 変更が必要＝起案止まり

## 構文分析のペン入力（実現可能性検証・2026-08-25）✅ 実装済み（要実機計測）

「手書きと同じ感覚の構文分析」（構文添削コーチ構想 v1.3）の第一歩。ペンで英文の上に書いた
括弧・下線・記号・文字をその場で判別して単語位置に吸着させる。ADR `20260825-pen-syntax-feasibility`。
**外部サービス・有料API不使用（判別はすべて端末内・0円）**。

- **純ロジック** `src/lib/pen-syntax/`（テスト＋計測ベンチ）: 幾何特徴（`geometry.ts`）・
  $P点群照合の自前実装（`pdollar.ts`）・形の記号判別（`shapes.ts`・丸/山括弧は円弧と2本線の当てはめ比較）・
  文字判別（`letters.ts`・**有限候補への当てはめ**＝上の行なら品詞の英字6種 n/v/a/ad/aux/p・下の行なら働き8種）・
  単語吸着（`snap.ts`・折り返し複数行対応・句読点除外）・手のひら判定（`palm.ts`）・
...
- **計測ページ** `/reading/syntax/pen-lab`: お題方式で判別・吸着の正誤を自動集計（二車線別）＋
  手のひらカウンタ＋**お手本登録**（本人の字を localStorage に保存して照合。漢字の判別率を上げる前提）
- **機械計測**（種固定・`benchmark.test.ts`）: 群A一発90.6%/救済込み99.2%・群B99.0%・
  働き99.1%・品詞（英字化後）一発98.9%/救済込み100%・吸着100%。**実機ペンの実測は塾長が pen-lab で行う**
- **実機フィードバック6点の改修（2026-08-25・ADR `20260825-pen-pos-letters`）**:
  ①下線=実測した単語の箱から行ごとに1本の連結線（`underlineSegments`）②太さ3px→1.5px
  ③カッコ=本文行の高さの箱で上下中央揃え ④入れ子カッコ=深さ別4色（`bracketDepths`・Okabe-Ito 配色・開閉同色）
  ⑤品詞=黄リー教式英字6種（採点は漢字正解表と `posLetter` 同値化・読解1文画面は漢字のまま・
  タップ方式は `SyntaxAnnotator` の `posOptions` プロパティで英字を注入）
  ⑥パーム対策=**「ペンのみ」既定**＋`usePenScreenGuard`＋お手本登録キャンバスにも同じポインタ判定＋
  拒否接触は preventDefault。ペンが反応しない端末の逃げ道は練習ページの「指・マウスでも書く」切替。
  **★2026-08-26 改修（ADR `20260826-pen-finger-scroll`）**: ガードを「ペンを一度見たら指を常時無効化」から
  **時間窓方式**（`src/lib/pen-syntax/finger-guard.ts`・`PEN_RELEASE_MS`=1.2秒）に変更。ペンの接近・接触中と
  離した直後だけ指を止め（手のひらを載せたまま書ける・止めた接触は離れるまで止め続ける）、
  窓が明ければ描画エリア外の指スクロールは普通に通す。ペン由来の互換タッチは常に通す（8/26 修正維持）・
  描画中の画面固定 `freezeScreenDuringStroke` は不変。「入力の記録」に通した/止めた別と理由を日本語で記録
- **★2026-08-26 第4便（ADR `20260826-pen-onboarding-calibration`）**:
  ①**記号の扱い3点**（模範分析集 第7講で裏取り）: 「M」を削除（`ROLE_LETTERS`=S/V/O/C/P/Po/▷）・
  「P」「▷」は働きの側（下の段）・「Po」を「前O」等に言い換えない（`roleLetterToAppRole` 廃止・
  構文の練習の働き語彙は `ROLE_LETTER_OPTIONS`、タップ方式へは `SyntaxAnnotator` の `roleOptions` で注入。
  練習3問の正解表も塾長表記へ＝修飾語に働きを書かない・前置詞は P・目的語は Po。矛盾検査も対応。
  **読解1文画面の `ROLE_OPTIONS`（M/前O/接）は既存データ・構文AIと揃えるため不変**）
  ②**初回お手本登録の義務化**: ペン方式の初回に必須21種（括弧8種は各2本＋品詞6＋働き7）の登録を
  必ず通す（`PenOnboarding`/`EnrollCanvas`/`onboarding.ts`・利用者ごと1回・完了印は localStorage に
  利用者IDつき・途中でやめたら続きから・登録し直しは練習ページ下部と pen-lab 下部・DB変更なし）
  ③**閉じ括弧の判別強化**: `classifyShape` が登録お手本を照合（向きゲート＋同じ向き4種の相対比較・
  4種全登録が条件・強い幾何特徴には大差を要求）。機械計測=癖のある閉じ括弧で一発 53.3→58.3%・
  救済込み 92.5→99.2%・標準的な書き手は 90.6→91.1%（劣化なし）。お手本なしの挙動は従来と同一
- **★2026-08-26 第5便（ADR `20260826-pen-foundation-hardening`）: 基盤の作り込み**。
  設計の正本は **`docs/pen-input-architecture.md`（設計の1枚）**＝ペン入力を触るセッションはまず読む。
  ①入力処理を `useStrokeCanvas` に一本化（4か所の重複解消・受理判定/座標変換/画面固定/記録を1か所へ）
  ②採寸を `useTokenBoxes` に一元化（毎描画後に自動で測り直し・条件の列挙を廃止＝下線ずれを構造で防ぐ）
  ③画面ガードを**ゾーン方式**へ（塾長裁定・`zone-guard.ts`＋`usePenZoneGuard`。書き込みエリア内=
  ペン専用／エリア外=指は待ちなしで有効・ペンは無効（ボタンは指で押す）／ペン接触中に載った
  手のひらは離れるまで止める。時間窓方式 finger-guard は削除）
  ④**記号台帳の確定版**（正本 `ledger.ts`）: 品詞 n/v/a/ad/aux（**p廃止**）・働き S/V/O/C/P/Po/▷/**＋**/同格・
  ○で囲んだ漢字の例外マーク（仮・真・強調・同格・`classifyExceptionMark`）・波線=熟語・
  深さは書かず括弧から自動判定（色＋自動ダッシュ印・`dash-notation.ts` で S′″‴ と相互変換=採点同値）・
  [｛の開始カッコ真下にまとまり全体の働き（`StudentSpan.role`）・台帳外（?/ダッシュ/Ø/単語囲み○）は
  反映せず案内・お手本必須は20種。
  **★2026-08-28: 括弧の見た目を半角にそろえた**（`( )` `⟨ ⟩`（U+27E8/U+27E9）`[ ]` `{ }`。
  丸括弧と山括弧だけ全角で幅が食い違っていた）。画面に描く字（`syntax.ts` の `SPAN_TYPES` の
  `open`/`close`）も同じ字にそろえ、説明の文とAIへ渡す文面はルールブックに合わせて全角のまま。
  記号の見分けは ID・種類の名前で行うので過去の記録は不変（ADR `20260828-symbol-halfwidth`・
  見張り `ledger.test.ts`）
  ⑤「入力の記録」を**再生テスト化**（`replay.ts`・「再生用をコピー」→ `replays/` に保存→テスト1件追加で
  実機不具合が再現テストになる。既知3件収録済み）
- **★2026-08-26 第6便（ADR `20260826-pen-order-hints`）: 分析の順序の記録と検討順ヒント**。
  「順序も大切な作業」（塾長）に基づく3点・追加費用0円・DB変更なし。
  ①**順序の記録** `src/lib/pen-syntax/order.ts`: ペン方式の記入・一手戻す・削除を時系列でため、
  採点時に「どの記号をどの順で書いたか」へ畳む（筆画の開始時刻から導出・同じマスの上書きは
  最後だけ残す・解答/採点データの形式は不変の付帯情報・控えは端末内 localStorage 直近30件）
  ②**模範筆順の採取口**: 講師は採点後「この筆順を模範の順序として保存」→ページ下部の一覧から
  JSONコピー（`ModelOrderPanel.tsx`・localStorage・塾長が採取して検討順の調整材料にする）
  ③**検討順ヒント** `order-hints.ts`:「💡 迷ったらまずこれ」ボタン。確定済みの記号から
  「まだの最初の項目」を1つだけ提示（正解表は見ない）。初期リスト=動詞→S→名詞の4役→
  カッコの検討順（ルールブック「カッコの見分け方」:［名詞］→｛ ｝→〈 〉→（ ））。
  リストは設定データ（`ORDER_HINT_RULES` の並び）で入れ替え可能。文面は
  「次に調べる対象と調べ方（辞書・ルールブックの引き先）」まで＝答えは言わない（構想v0.3の3原則）。
  記録の通知口は `PenSyntaxAnnotator` の任意プロパティ `onOrderEvent`（共有部品 SyntaxAnnotator は不変）
- **★2026-08-27 第10便（ADR `20260827-pen-symbol-layout`）: 記号を付けても書く場所が
  動かないようにする**。塾長の実機の指摘「下線やカッコを引いたときに書き込もうと
  していた領域自体が移動する」「下線はもっと文字にピッタリ出したい」への対処。
  ①**カッコ（開き・閉じ・閉じ待ち）を単語の並びから外して重ね描き**（`snap.ts` の
  `bracketMark`。単語のすき間の中央・実測12〜19画素・入れ子は `BRACKET_SLOT_W`=6 ずつ
  左右へ・文字は24→20画素）。記号を足しても後ろの単語が動かない
  ②開始カッコの真下の働きの判定に**縦位置**を足す（折り返しのある文で別の行の
  カッコに吸われない・審査の指摘）③**下線は文字のベースラインのすぐ下**
  （`TokenBox.baseline`＝語末に置いた中身の無い inline-block の下端を採寸。
  q・y・g・p は突き抜けてよい＝塾長の明示の希望）④**○で囲んだ例外マークは働きの欄へ**
  （`roleCellParts`。仮・真は働きと組んだ1つの値「仮S」／強・同は単独で1マス。
  並びは台帳の順＝書く順序に左右されない）⑤**例外マークは1字**（`EXCEPTION_KANJI`＝
  仮・真・強・同。決定記録 `20260827-syntax-symbol-clarifications` の1）。構文の練習の
  働きの選択肢も「同格」→「同」（読解1文画面の `ROLE_OPTIONS` は不変）。
  縦横の位置は**表示専用**（吸着・採点に無関係）
- **★2026-08-27 第11便（ADR `20260827-pen-role-band-and-colors`）: 働きの高さ・階層の色・
  採点後の並び**。塾長の本番の評価「かなりよい」のうえで挙がった3点。
  ①**同じ行の働きは同じ高さの帯にそろえる**（`snap.ts` の `ROLE_ROW_H`=24画素に1本化。
  カッコの真下の働きも単語の下のマスと同じ上端・同じ高さ・同じ文字の大きさで中央そろえ。
  従来はカッコ側だけ上端そろえ・11画素で中心が4.1画素ずれていた）
  ②**入れ子の深さの色を、隣り合う深さが青と橙（ほぼ補色）になる並びへ**
  （**2026-08-28 に4色とも組み直し**: 0=濃い青 `#003368` / 1=濃い橙 `#4D2801` /
  2=明るい青 `#0074DE` / 3=明るい橙 `#A16900`。白地に対する明暗の比が4色とも 4.5:1 以上に
  なった（直す前は明るい2色が 3.51・3.86）。ADR `20260828-bracket-color-contrast`・
  計算は `node scripts/bracket-colors.mjs`・見張りは `bracket-colors.test.ts`。
  同じ側の色は濃さで分ける。赤と緑の見分けに頼らないので色覚の型によらず差が残る＝
  いちばん近い組み合わせの色の差は従来 T型で1・D型で19 だったのが29以上に）
  ③**採点の指摘を単語の並びから外し、働きの欄のすぐ下へ重ねて描く**（`snap.ts` の `noteMark`。
  文面は「品詞 n」の形に短くして1マスに収める。置き場所は採点していないときも
  下余白として常に確保＝`pb-7`。直す前は採点した瞬間に単語が最大81.94画素ずれていた）。
  変更はすべて表示だけ（吸着・採点・保存データは不変）。共有部品 `SyntaxAnnotator` は不変
- **★2026-08-31（ADR `20260831-pen-ux-refinements`）: 書き味の改修5件＋採点データの一本化**。
  ①**例外の印（仮・真・強・同）の○手書き認識を廃止し、タッチ選択式へ**
  （`recognizeGroup` から `classifyExceptionMark` を呼ばない＝入口のみ閉鎖。
  仮・真=判定済みの S/O のマスから・強=働きのマスの選択肢・同=働きの一覧の「同」。
  お手本の任意登録・計測のお題からも除外）。あわせて入力経路による採点の割れを一本化:
  **仮・真・強=extras（採点対象外・正解表に定義なし・強は働きと同じ単語に共存）／
  同=answer.role（採点対象・正解表の「同格」と roleBase で同値）**。
  付け外しの純関数 `toggleExceptionMark`/`pruneExceptionMarks`（働きが S/O 以外になると仮・真は外れる）
  ②**候補が未確定のまま次の字を書いたら最有力候補で自動確定**（従来は破棄され判定が失われた。
  書き始めで候補を取り置き→続けて書いた=自動確定（記録に残す）／枠の外のタップ=従来どおり破棄。
  決定記録 20260827-pen-chip-interrupt の「受付を止めない・当たり判定の分離」は維持）
  ③**働きの記号は下線の塊のどの単語の下に書いても塊の最後の単語に付く**
  （`roleTargetIndex`・末尾の句読点は除く・タッチでマスを直接選んだときは付け替えない）
  ④**行またぎ下線は3条件（前行の行末まで達している＋働き未記入＋次行の行頭から）で連結**
  （許容誤差は語単位=句読点のみ無視。`applySymbol` に任意引数 `{tokens, allBoxes}` を追加）
  ⑤**入れ子カッコは縦にも8pxずらして斜めに離す**（`BRACKET_SLOT_H`・外側ほど上・4色は不変）
- **★2026-09-01（ADR `20260901-syntax-grading-modes`）: 採点ルールの拡張と通常/MAXモード**。
  ①正解表のまとまりに**「任意」の区分**（書いても書かなくても減点しない・`KeySpan.optional`）:
  「very well」型＝副詞が副詞を前置修飾する塊への下線＋丸カッコの流儀を受理
  （書かれたら種類を確かめ得点は動かさない・種類の誤りは従来どおり誤り・
  未記入は見落としにしない・正解表示に含めない・同範囲の必須と任意は種類の合う行を優先対応）。
  練習①の very well に下線を任意登録＋模範分析集 第7講の同型3文
  （P3-S1 ever more／P4-S4 so much＝下線のみ／P8-S2 not simply）を
  **コード側の表**（`syntax-instructor.ts`・生成ファイルは取り込み直しで消えるため）で登録
  ②**採点モード**（`gradeSyntax(problem, answer, mode)`・既定 normal）:
  通常=品詞（n/v/a/ad/aux）を求めない（未記入素通し・書けば指摘だけ・得点外）。
  **前置詞・接続詞の専用記号 P・Po・▷・＋は働きの欄なので通常でも従来どおり採点**。
  MAX=従来どおり品詞まで。構文の練習の画面に切替（採点後の切替はその場で採点し直し）
  ③**筆順登録の品詞5種を必須から任意へ**: 必須=括弧8種＋働き7種の**15種**
  （`ledger.ts` REQUIRED_SYMBOLS。品詞は OPTIONAL_SYMBOLS へ・登録したい人だけ）
- **★2026-09-01（ADR `20260901-pen-stroke-accumulation`）: 手書き判定の精度向上（実書き蓄積＋見分け強化）**。
  方式は文字認識検討会（2026-08-31・統合塾運営エージェント `docs/proposals/2026-08-31-文字認識検討会記録.md`）で確定。
  ①**実書き蓄積（本命）**: 確定して訂正しなかった線をお手本としてDBへ自動蓄積
  （`027_pen_stroke_samples.sql`＝本人の蓄積＋**名前を付けない**塾の共通お手本集の2表・
  `/api/pen-samples`・`sample-store.ts`・`usePenTemplates`。上限=本人16/共通24件・
  形を保つ間引き・関所=取り消し/削除/値の変更で外し**採点時に**送る・候補タップの線は最良の教師。
  端末内お手本は初回にDBへ移行・オフライン/未適用時は従来どおり）
  ②**見分け強化（補強）**: 括弧の「違いの出る部分」（へりの平らさ=角括弧・幅の狭い突き=波括弧）・
  本人のお手本と意見が割れたら候補選びへ・働きS/▷/＋/Poと品詞a/ad/auxの幾何裏付け・波線の緩和
  ③**取り違えゼロ側**: 確定に確信の下限（形0.7・文字0.55・`tuning.ts` が正本。
  着手前の挙動は `LEGACY_TUNING` で保存＝同じ線で前後比較可）。続け書きの自動確定にも同じ下限・
  下限未満は破棄せず保留列に残す ④**吸着**: 括弧は単語の中央を境界に（重ねた閉じ括弧が前後しない）
  ⑤**計測**: 3つの数字（一発確定・取り違え・候補選び）を `metrics.ts`/`accuracy.test.ts`。
  実書きは pen-lab の「実書きデータをコピー」→`src/lib/pen-syntax/samples/*.json` で自動前後比較。
  合成データ（実機の実測ではない）の総合計: 一発74.2→75.9%・**取り違え5.7→1.1%**（標準の書き手は83.5%/0.6%）
- **★2026-09-02（ADR `20260902-pen-autoconfirm`）: 全記号の自動確定への転換＋波線・タッチ修正・カッコ描画（5件）**。
  塾長の実機の使用感による方針転換（検討会 2026-08-31 の一部を上書き）。
  ①**全記号の自動確定**: 判別の最有力候補があれば迷っていても確定・表示（候補チップは
  「拾えず」のときだけ）。誤りはタッチで修正。**蓄積の関所**=迷いのある確定（ambiguous）と
  タッチで訂正された線は実書き蓄積に入れない（訂正は「判別の訂正」と「分析の考え直し」を
  区別できないため採り直さない）。「一手戻す」＝末尾1件の対応は skip 印の場所取り行で維持。
  計測（`metrics.ts`）は 'chips'/'auto' の2つの読み（auto の取り違え=修正されず残った誤り）
  ②**波線**: 筆順登録の必須へ昇格（括弧8＋働き7＋波線=**16種**。登録済みの利用者には
  missingRequired の続き再開で波線だけの追加登録が自動で出る）＋描画を下線と同じ
  連結線分＋波形SVG（`wavyPath`・行またぎ連結は下線の3条件から働き条件を除いた2条件）
  ③**下線のタッチ修正**: 下線・波線のタッチで種類の変更（下線↔波線）・削除
  （`findLineAt`・重なりは波線優先。範囲の変更は含めない=削除して書き直す）
  ④**働きのタッチの2分岐**: 「修正（値の変更・削除）」「印の追加（仮・真・強）」を先に選ぶ
  （「同」は修正の一覧のまま・「一覧から選ぶ」の入力し直しは分岐を飛ばす）
  ⑤**カッコの重ね描きを自前のSVG字形へ**（`bracketGlyphPath`・8×20pxの同じ箱・閉じは左右反転）:
  日本語フォントの丸括弧の字面（大きく基線より下）と ⟨ ⟩ のフォント落ちで同じ組でも
  大きさ・基線が食い違い、縦ずらし8pxと相乗りして重なりが読めなかった（吸着に誤りなし）。
  フォント非依存で4種の大きさ・基線が完全一致し、段差は意図した8pxちょうどになる
- 本編1文画面への搭載・迷い文の復習カード化はスコープ外（実測を見て別途起案）

## 模範分析集 第7講の35文を構文の練習に取り込む（2026-08-27・講師限定）✅ 本番反映済み

塾長が模範筆順（分析をどの順で書くか）を採るための文を増やす。ADR `20260827-instructor-syntax-problems`。
共有事項 **C24**（模範分析集の受け渡し）。
- **取り込み** `data/sync-syntax-problems.mjs`: quiz_generator の
  `subjects/英語/模範分析集/export/構文の練習_英語長文最前線_第7講.json` を**読み取り専用**で読み、
  `private/syntax-problems/英語長文最前線_第7講.json` を生成（C23 と同じ向き・quiz_generator には書き込まない）。
...
- **講師限定（2026-08-28 にサーバー側の守りへ）**: 正解表の実物は **`private/syntax-problems/`**
  （配信されない場所）に置き、入口 **`GET /api/reading/syntax-problems`** が `requireTeacher` で
  **`profiles` の役割を確かめてから**返す（未ログイン401・生徒403）。画面から送られた役割は信じない。
  画面側の `syntaxProblemsFor(base, instructor, isTeacher)`（`syntax-instructor.ts`）は見せ方だけの役割で、
  生徒は従来の練習3問のまま＝**自動テストで固定**。読み込みは `syntax-instructor-load.ts` が入口から取りに行く。
  端末や中継に残さない指定＋検索よけ付き。ADR `20260828-instructor-key-teacher-gate`・
  見張り `src/lib/reading/syntax-problem-access.test.ts`（9件）
  ※ 2026-08-27 までは正解表（約174キロバイト）が画面のコードに同梱されており、
  後から読み込む形にしても誰でも取りに行けた
- **画面**: 問題の一覧が「練習（3問）」と「模範分析集 第7講（講師用 35問・生徒には出しません）」の2組。
  問題を開くと講師用の注意書きと**落とした記号の注記**（全64件・各問の `key.notes`）を表示
- **採点の表記のゆれ**: 深さの印（`S′`）と2字の「同格」を `roleBase` で同値に扱う（許容解の新設ではない）
- **注意**: この35問は**生徒に出せない**（記号の一部が落ち許容解も無い）。短縮形 `That's`・割合 `15%` は
  1語に2つの値（「n v」「S V」「a n」）が入っており画面では書けない＝その語はどう書いても誤りになる
- **★2026-08-27 第7便（ADR `20260827-pen-commit-latency`）: 続けて書いた記号が
  1つにまとまる問題の解消**。塾長の実機フィードバック「書き込んでから読み取られて
  描画されるまでの間、すべてが1つの記号として認識されてしまう」への対処。
  ①**触れた瞬間に境界（行・段・単語）をまたいでいれば、0.75秒待たずに確定**する
  （`useStrokeGrouping`。1点しか分からないので確信のある根拠だけで切る。括弧は
  本文の上端より上から書き始めるため段の判定に余裕を取り、際どい位置は切らない）
  ②まとめ判定を `grouping.ts` に集約（`snap.ts` から移動）し、**場所の比較相手を
  「まとまり全体の外接箱」から「直前の1画」へ**変更。切る根拠は
  間があいた→行→段→横長の線（下線・波線は1画で完結）→単語をまたいだ→
  単語がちがう→記号として大きすぎる→離れている、の順。**同じ単語の同じ段で
  書き続けている間は待つ**（○囲みの漢字・2画のS/V・括弧の書き直しを切らない）
  ③書き込み部品と計測ページに複製されていた判定・待ち時間・確定を一本化
  （`useStrokeCanvas` の `onStrokeStart` が書き始めの点を渡す）
  ④**速さの計測を新設**（従来の計測は正誤だけ）: 確定のたびに「入力の記録」へ
  待ち時間を残し、計測ページに「確定までの時間（速さの計測）」欄。合成データでは
  まとまりの一致率 新100%/旧58.1%、次を書き始めてから前が確定するまで 新0ms/旧140ms
  （**実機の実測ではない**）。⑤`replayGrouping` と症状の記録を収録（再生テスト）。
  待ち時間 0.75 秒は据え置き（短縮すると画数の多い記号が途中で切れるため）
- **★2026-08-27 第8便（ADR `20260827-pen-chip-interrupt`）: 候補が出ている間に
  書いた線が消える問題の解消**。第7便で確定が早まった副作用として、判別に迷ったときの
  候補が書いている最中に出るようになったが、**候補が出ている間は書き込みの受付が止まる**
  作りのままだったため、テンポよく書くと次の記号の1画目が黙って消えた
  （＝2026-08-26 の「ペンが反応しない」と同じ体験）。
  ①`active` から候補の有無を外し、**書き始めたら候補を閉じてその1画目からそのまま描く**
  （閉じた候補は取り消しと同じ扱い）②「候補を押す操作」と「書き始め」は
  **当たり判定で分ける**（候補の枠 `z-20` はキャンバス `z-10` より上＝押した接触は
  キャンバスに届かない。時間・距離のしきい値を増やさない）③枠の外の軽いタップは
  閉じるだけ（品詞・働きの一覧は開かない）④計測ページ（pen-lab）にも同じ扱い。
  ただし書いて閉じた線は**計測に数えない**（お題は据え置き）⑤閉じたことは
  「入力の記録」に1行残す。テスト3件を新設（この経路には従来テストが無かった。
  うち2件は修正前のコードで失敗することを確認）
- **★2026-08-27 第9便（ADR `20260827-pen-write-as-on-paper`）: 紙のとおりに
  書けるようにする3点**。塾長の実機確認で見つかった「書きたいように書けない」箇所。
  ①**閉じる前の開始カッコの真下にも働きを書ける**（`PendingOpen.role`＝閉じ括弧が
  来たときに `StudentSpan.role` へ引き継ぐ。閉じ待ちは閉じ済みより優先・同じ位置の
  閉じ待ちが複数なら後に書いたほう＝内側）。画面にも閉じ待ちの下に働き欄の枠を出す。
  あわせて「まとまり全体の働きか」の判定を**吸着した単語から単語のすき間へ**変更
  （カッコの列は実測12画素しかなく、そこに書いた線は左どなりの単語のほうが中心に
  近いため黙って単語の働きに入っていた。前後6画素の見込み・行頭は40画素まで）
  ②**お手本の保存を端末ごとから利用者ごとへ**（`userTemplatesKey(userId)`。
  利用者の付いていない古い保存は読み込まない＝他人の字を引き継がない。
  「完了の印」は廃止し `needsEnrollment`＝本人のお手本がそろっているかで判定。
  構文の練習の下部に「お手本の登録: N / 20 種」を表示）
  ③**候補の枠は、いま書いた場所を避けて出す**（`chip-place.ts` の純関数＋
  共通フック `useChipPlacement`。品詞の段に書いたら行の下・働きの段なら行の上、
  避ける範囲は「書いた線＋その行の本文」、左右のはみ出しを抑え、上下どちらも
  枠に収まらないときは枠の外（下）へ逃がす。書き込み部品と計測ページの両方）。
  DB変更なし・追加費用0円。実機（タブレット＋ペン）の確認は塾長待ち

## 教材の本文はログインした人だけが読める（2026-08-27）✅ 本番反映済み（2026-08-28）
