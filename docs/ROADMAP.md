...
**Phase**: コロケーション中心デッキ パイロット実装完了（語義A1/A2統制＋例文ローテーション）。次は実機検証
**最終更新**: 2026-06-14
**次のタスク**: デプロイ後パイロット実機検証 → 評価後に拡大/精緻化

### 次回セッションでやること

1. **Phase 10 着手**: 記憶のいきもの育成（@docs/memory-creatures-design.md）。まず 10.1 いきものステータス基盤（card_states → 成長/世話状態の純ロジック＋テスト）から
2. **未決定論点を詰める**: 種の割り当て方／成長段階しきい値／インプリント入力手段／墓場UX／スプライトのアートスタイル
3. **実機検証（手作業・並行）**: デッキ「中学英単語 コロケーション（パイロット50語）」で例文ローテーション＋頭子音ヒント空欄を確認
