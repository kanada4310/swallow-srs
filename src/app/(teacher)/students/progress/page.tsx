    ...
    <AppLayout>
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold text-gray-900">生徒取組状況</h1>
          <Link
            href="/students"
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 text-center">
            ...
            <div className="text-xs text-gray-500">今日学習済み</div>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 text-center">
            <div className="text-2xl font-bold text-orange-600">
              {students.filter(s => s.dueCards > 0).length}
            </div>
            <div className="text-xs text-gray-500">未復習あり</div>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 ml-4 text-sm">
                  <div className="text-center">
                    <div className={`font-bold ${student.reviewsToday > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                      {student.reviewsToday}
                    </div>
                    <div className="text-xs text-gray-400">今日</div>
                  </div>
                  <div className="text-center">
                    <div className={`font-bold ${student.dueCards > 0 ? 'text-orange-600' : 'text-gray-400'}`}>
                      {student.dueCards}
                    </div>
                    <div className="text-xs text-gray-400">期限切れ</div>
                  </div>
                  <div className="text-center">
                    <div className={`font-bold ${student.overallAccuracy >= 80 ? 'text-blue-600' : student.overallAccuracy >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
