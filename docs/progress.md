...
## 現在の作業
- Phase: コロケーションパイロットに **文脈アシスト（日本語の場面リード文）** を追加完了 → 次は Phase 10「記憶のいきもの育成」が主軸（設計メモ済み・コード未着手）
- 最終更新: 2026-06-14
- 次にやること:
  1. **要デプロイ／再同期**: 文脈アシストは `StudyCard.tsx` 改修（Vercel デプロイ必須）＋テンプレ/例文プールのDB更新（再同期で反映）。デプロイ後に実機確認
  2. **Phase 10 着手**: 記憶のいきもの育成（一次仕様 @docs/memory-creatures-design.md）。まず 10.1 いきものステータス基盤（`card_states` → 成長段階/世話状態を導出する純ロジック＋テスト）から
  3. **未決定論点を詰める**: 種の割り当て方／成長段階しきい値／インプリント入力手段／墓場UX／手続き生成スプライトのアートスタイル
  4. **その後**: Phase 13.4 リッチコンテンツ表示（数式 KaTeX/MathJax・画像フィールド）→ 数学・理科へ科目拡張
  5. パイロット評価後: OOV例文リペア / 全語へ拡大 / 本格コーパス補強 / 2286語デッキへ空欄スタイル適用 / 文脈アシストを全語に展開
  6. （完了）実機検証: 例文ローテーション＋頭子音ヒント空欄（前セッションの残タスク）→ 確認済み・問題なし
