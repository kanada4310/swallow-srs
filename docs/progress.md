...
- 最終更新: 2026-06-16
- **画像マスキングは実機確認済み**：Google Vision 高精度OCR・手直しUX・毎回ランダム出題・オフライン・裏面リスト表示・一括作成・ビジュアル再編集まで動作確認済み
- 次にやること:
  1. **一括作成/再編集の実機確認**（軽め）: デッキ詳細「一括マスキング」で複数枚→レビュー→全作成、ノート一覧の鉛筆でビジュアル再編集
  2. **科目拡張**: 数式＋画像マスキングが使えるので数学・理科デッキ作成 → そこの「いきもの」も飼える（Phase 10 連携）★次の主軸候補
