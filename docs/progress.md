...
## 現在の作業
- Phase: **古文単語演習の表示改善（漢字非表示・両タグ）＋単語帳「定着度ビュー」✅**（2026-07-07・`f31a66c` コミット/プッシュ済み・**要 Vercel デプロイ＋再ログイン**）＋デザイン全面刷新✅＋チュートリアル第1弾✅
- 最終更新: 2026-07-07
- 次にやること:
  1. **今回分の実機確認（デプロイ後）**: 古文単語315 デッキ詳細の「単語帳（定着度）」タブ／モードA の漢字非表示／解答表示の「あなたの解答」＋「正しい選択肢」両タグ。定着度しきい値（`src/lib/wordbook/mastery.ts`）を実データで調整
  2. **Vercel デプロイ＋実機確認**: 生徒スマホ（LINE経由）で新デザイン・ミッションカード・コーチマーク。`scripts/device-preview.mjs` でも随時確認可
  2. **チュートリアルデッキの配布**: 「つばめSRSのつかいかた」(id `12347515…`) を講師UIからクラスへ配布（現在はテスト生徒のみ）
  3. **総合評価レポートの改善実装**: 最優先=**learn-ahead（学習ステップ待ち時間の解消）**→ 練習モードの記録方式 → 速度→簡単判定の重み → 同期差分化 → 送信キューのデータ消失対策
  4. 古文単語演習・識別演習の実機確認（自動判定しきい値の実データ調整）
- **古文単語演習（2026-07-07 コミット済み）**: 6/22 実装分を検証（テスト397件・build 成功・DB投入済み=デッキ904ノート＋ノートタイプA/B＋モード別サブデッキ2つ）の上でコミット・プッシュ。**要実機確認**（`/study`「古文単語315」）。※G:ドライブの `古文単語315.json`（6/19版）は related フィールドが旧仕様の**古い版**。インポートに使った正は `quiz_generator/data/古文/古文単語315.json`（6/20版・related はインポート未使用なので出題内容は同一）
