  ...
  if (!stats) {
    return (
      <div className="bg-white rounded-card border border-gray-200 p-8 text-center">
        ...
        <h2 className="text-lg font-bold text-ai mb-2">統計データがありません</h2>
    ...
    <div className="space-y-6">
        ...
        <div className="inline-flex rounded-2xl border border-gray-200 bg-white p-1">
          {PERIOD_OPTIONS.map((option) => (
            <button
              ...
              className={`px-3 py-1.5 text-sm font-bold rounded-xl transition-colors ${
                period === option.value
      ...
      {stats.timeStats && stats.timeStats.totalReviewTime > 0 && (
        <div className="bg-white rounded-card border border-gray-200 p-4">
          <h3 className="text-sm font-bold text-ai mb-3">学習時間</h3>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs text-ink-3">合計学習時間</p>
              <p className="text-lg font-extrabold text-ai tabular-nums">
                {formatTime(stats.timeStats.totalReviewTime)}
              </p>
            </div>
            <div>
              <p className="text-xs text-ink-3">平均回答時間</p>
              <p className="text-lg font-extrabold text-ai tabular-nums">
