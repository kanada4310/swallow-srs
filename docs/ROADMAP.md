...
- [ ] **専用ページ化（/notes）**: デッキ一覧埋め込みから独立ページに移行
...
**Phase**: Phase 7.6 ノートブラウザ強化 + コピー/移動（作業中）
**最終更新**: 2026-02-06
**次のタスク**: ノートブラウザ専用ページ化
...
1. **ノートブラウザ専用ページ化（最優先）**:
   - `/notes` 専用ページを作成（デッキ一覧埋め込みから移行）
   - ナビゲーションメニューに「ノート」ボタン追加（デッキの次）
   - 全ノート検索の結果が少ない問題を調査・修正
     - ログインユーザーが所有するデッキIDのリストを確認
     - RPC search_notesに渡すdeck_idsの内容をログ出力して検証
   - DecksPageClientの簡易検索UI・ブラウザ埋め込みを撤去
2. **Phase 7**（ユーザー要望、続き）:
...
- **全ノートブラウザで検索結果が少ない**: deckId未指定時にsearchDeckIdsが限定的。RLSまたはデッキ所有者の問題の可能性。要調査
