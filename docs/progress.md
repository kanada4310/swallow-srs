...
## 現在の作業
- Phase: 中学英単語 暗誦例文デッキ作成 + 同期ページングバグ修正完了 → 次は金田アカウントで動作確認
- 最終更新: 2026-06-13
- 次にやること:
  1. **動作確認（要デプロイ後）**: Vercel に `2c53f0f` がデプロイ完了 → アプリで**ログアウト→再ログイン**（clearAllData で IndexedDB 全消去 → フル同期）→ 品詞別10サブデッキ＋穴埋めカードを確認
