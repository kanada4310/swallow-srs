...
デッキごとにAnki本家相当の学習設定（学習ステップ、間隔倍率、失念の多いカードの検知等）をカスタマイズ可能に。
...
- [x] scheduler.tsリファクタリング（全関数にsettingsパラメータ追加）
  ...
  - 失念の多いカードの検知（checkLeech: threshold+suspend/tagアクション）
...
- [x] answer API更新（デッキ設定読み込み、lapses保存、失念の多いカードの検知・tag/suspend）
- [x] sync.ts更新（saveAnswerLocallyにlapses対応）
- [x] StudySession更新（deckSettings prop、失念の多いカードの通知トースト、suspend対応）
...
**Phase**: **生徒体験の穴埋め4機能＝学習開始のデッキ選択／フィルタサブデッキ表示／サブデッキ配布／生徒別設定**（2026-07-08 完了・022/023 適用済み・デプロイ済／実機確認は一部）
**最終更新**: 2026-07-08
**次のタスク**: 実機での残検証（講師の生徒別設定→生徒反映・選択モーダルの実端末タップ・継承配布サブデッキ）→ 科目拡張（数学・理科デッキ）→ 各種しきい値の実データ調整

### 次回セッションでやること

1. **実機での残検証**: 講師（実アカウント）で生徒詳細⚙→学習設定変更→テスト生徒の再同期で反映／ホームの選択モーダルの実端末タップ／継承配布サブデッキの学習・削除。動作確認用デッキ「（動作確認用）くだもの」の掃除
2. **科目拡張**: 数式＋画像マスキングで数学・理科デッキを作成（Phase 10/13.4 連携）／コロケーション文脈アシスト残825（API月次上限の回復後に top-up）
3. **各種しきい値の実データ調整**: 単語帳の定着度／古文単語演習／識別演習の自動判定
4. **運用メモ**: Supabase CLI は再ログイン済みで `db push` 復旧（020〜023 は `migration repair` で履歴記録済み）。将来 login 切れ時は SQLエディタ貼り付けで代替可
...
### 完了済み
- [x] **生徒体験の穴埋め4機能**（2026-07-08・022/023 適用済み・デプロイ済／実機確認は一部）: ①ホーム学習開始でデッキ別内訳＋複数時は選択モーダル（`StudyDeckPicker`）②フィルタサブデッキ詳細の単語帳・ノート・統計を親ツリー＋タグ絞りで表示（`src/lib/db/deck-scope.ts`・テスト10件）③サブデッキも一緒に配布（`deck_assignments.source_deck_id`＋バックフィル49件・`022`）④配布デッキの学習設定を講師が生徒ごとに変更（`user_deck_settings` 講師RLS `023`＋`/api/teacher/student-deck-settings`）。既存バグ修正: notes/search のクラス配布403／非オーナー設定のサブデッキキー。ADR `20260708-subdeck-assignments` / `20260708-teacher-student-deck-settings`
- [x] **学習の待ち時間解消（learn-ahead）**（2026-07-07・デプロイ＋実機確認済み）: 学習ステップの待機画面で操作がブロックされる問題を Anki の learn-ahead（既定20分）で解消。main キュー空でも次の学習カードが20分以内なら前倒しで出す。純ロジック `src/lib/srs/session-queue.ts`（`pickFromQueues`・テスト8件）＋`StudySession` から呼ぶ。スケジューラ不変。ADR `20260707-learn-ahead`
- [x] **開発運用ルールを CMS 方式に移植**（2026-07-07）: CLAUDE.md にコミュニケーションルール＋開発運用ルール／progress.md を索引＋ハンドオフ個別ファイル方式に分割／ADR（`docs/decisions/`＋索引＋再生成スクリプト）／`/check` コマンド。ADR `20260707-ops-rules-cms`
- [x] **単語帳（定着度ビュー）＝マイ単語帳**（2026-07-07・要実機確認）: デッキ詳細のノート一覧にタブを追加。`card_states` から5段階の定着度（未学習/苦手/学習中/定着中/定着済み）を導出し苦手単語を可視化・フィルタ・検索・タップで即復習。全デッキ共通・オフライン・純表示層（`src/lib/wordbook/`＋`WordbookView`・テスト13件）。詳細は CLAUDE.md「単語帳（定着度ビュー）」節
- [x] **古文単語演習の表示改善**（2026-07-07・要実機確認）: モードA 出題中の漢字（読み仮名）非表示＋解答表示で「あなたの解答」＋「正しい選択肢」両タグ表示（A/B共通）
- [x] **デザイン全面刷新「藍・空・喉の橙」**（2026-07-07）: docs/design-system.md＋全画面再スキン（カード内部の世界観は独立のまま）＋PWAアイコン新調。詳細は CLAUDE.md「デザインシステム」節
- [x] **チュートリアル第1弾**（2026-07-07）: デッキ「つばめSRSのつかいかた」15枚＋コーチマーク（初回フリップ時の評価ボタン説明）＋新規カード「順番どおり」出題の安定ソート修正
- [x] **生徒スマホ確認環境**（2026-07-07）: `scripts/device-preview.mjs`（Playwright＋本物の LINE 認証経路でテスト生徒に自動ログイン・デバイスエミュレーションでスクリーンショット）
- [x] **古文単語演習（tango.html モードA/B）**（2026-06-22 実装・2026-07-07 コミット）: 904ノート・自動判定・要実機確認
