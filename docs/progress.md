...
## 現在の作業
- Phase: 機能拡張ディスカッション完了 → **Phase 10「記憶のいきもの育成」を次の主軸に確定**（設計メモ作成、ロードマップ再構成済み・コード未着手）
- 最終更新: 2026-06-14
- 次にやること:
  1. **Phase 10 着手**: 記憶のいきもの育成（一次仕様 @docs/memory-creatures-design.md）。まず 10.1 いきものステータス基盤（`card_states` → 成長段階/世話状態を導出する純ロジック＋テスト）から
  2. **未決定論点を詰める**: 種の割り当て方／成長段階しきい値／インプリント入力手段／墓場UX／手続き生成スプライトのアートスタイル
  3. **その後**: Phase 13.4 リッチコンテンツ表示（数式 KaTeX/MathJax・画像フィールド）→ 数学・理科へ科目拡張
  4. **実機検証（手作業・並行）**: デッキ「中学英単語 コロケーション（パイロット50語）」で例文ローテーション＋頭子音ヒント空欄を確認（前セッションの残タスク）
  5. パイロット評価後: OOV例文リペア / 全語へ拡大 / 本格コーパス補強 / 2286語デッキへ空欄スタイル適用
