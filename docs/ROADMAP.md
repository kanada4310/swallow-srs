...
**Phase**: **古文単語演習の表示改善＋単語帳「定着度ビュー」**（2026-07-07 完了・`f31a66c`・要デプロイ）＋デザイン全面刷新＋チュートリアル第1弾
**最終更新**: 2026-07-07
**次のタスク**: Vercel デプロイ＋実機確認（単語帳タブ・モードA 漢字非表示・両タグ／定着度しきい値調整）→ チュートリアルデッキ配布 → SRS総合評価レポートの改善実装（最優先=learn-ahead 待ち時間解消）→ 科目拡張（数学・理科デッキ）
