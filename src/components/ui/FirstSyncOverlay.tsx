...
export function FirstSyncOverlay() {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-paper/95 backdrop-blur-sm flex flex-col items-center justify-center px-6"
      role="status"
      aria-live="polite"
    >
      <div className="bg-white border border-gray-200 rounded-card shadow-card px-8 py-6 flex flex-col items-center max-w-sm w-full">
        <svg className="w-10 h-10 animate-spin text-sora mb-4" fill="none" viewBox="0 0 24 24">
        ...
        <p className="text-ai font-bold">データを取得しています…</p>
        <p className="text-ink-3 text-sm mt-1 text-center">
          初回起動のため、デッキデータをダウンロードしています。
        </p>

        {slowMode && (
          <div className="mt-6 text-center">
            <p className="text-sm text-hard mb-2">少し時間がかかっています。</p>
            <button
              onClick={() => window.location.reload()}
              className="px-4 py-2 text-sm font-bold text-white bg-sora rounded-2xl hover:bg-sora-dark transition-all active:scale-95"
            >
              ページを再読み込み
            </button>
          </div>
