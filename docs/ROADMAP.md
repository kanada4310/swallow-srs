...
## Phase 10: ゲーミフィケーション・モチベーション

### 10.1 学習ストリーク + ヒートマップカレンダー
GitHub風の学習ヒートマップとストリーク（連続学習日数）表示。

- [ ] review_logs から日別学習量を集計
- [ ] ヒートマップカレンダーコンポーネント（ダッシュボード）
- [ ] ストリークカウンター（「15日連続学習中!」）
- [ ] ストリーク途切れ防止のプッシュ通知（Phase 12.3と連携）

### 10.2 デイリーゴール + 進捗リング
1日の学習目標と達成度をリング型プログレスで表示。

- [ ] ユーザー/デッキ単位のデイリーゴール設定（枚数 or 時間）
- [ ] 円形プログレスリングUI
- [ ] 講師がクラス/生徒に最低目標を設定可能

### 10.3 クラスリーダーボード
クラス内のランキング表示（学習量、ストリーク、正答率）。

- [ ] クラス内集計クエリ（review_logs × class_members）
- [ ] リーダーボードUI（週間/月間切り替え）
- [ ] 講師ダッシュボードに表示
- [ ] プライバシー配慮（オプトアウト設定）
...
- [ ] バッジ定義（初回100レビュー、7日連続、500枚マスター等）
- [ ] 達成条件チェック（レビュー完了時）
- [ ] プロフィールにバッジ表示
...
1. **実機検証**: デッキ「中学英単語 コロケーション（パイロット50語）」で例文ローテーション＋頭子音ヒント空欄を確認
2. **パイロット評価後**: OOV例文リペア / 全動詞or全語へ拡大 / 本格コーパス補強 / 2286語デッキへ空欄スタイル適用
3. **Phase 12.4**: billing側のLINE送信ジョブ
