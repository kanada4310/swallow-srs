...
- 最終更新: 2026-07-07
- **古文単語演習（2026-07-07 コミット＆デプロイ）**: 6/22 実装分を検証（テスト397件・build 成功・DB投入済み=デッキ904ノート＋ノートタイプA/B＋モード別サブデッキ2つ）の上でコミット・プッシュ。**要実機確認**（`/study`「古文単語315」）。※G:ドライブの `古文単語315.json`（6/19版）は related フィールドが旧仕様の**古い版**。インポートに使った正は `quiz_generator/data/古文/古文単語315.json`（6/20版・related はインポート未使用なので出題内容は同一）
