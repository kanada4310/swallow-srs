...
**1ノート＝1匹の生き物**を飼育する育成ゲーム。全体で1キャラを育てるのではなく、
無数の小さな生命を個別に世話する。生死・成長はすべて既存の FSRS `card_states` から
導出する（学習エンジンは不変、上にコスメティック層を被せるだけ）。実装はこのPhaseを先行。

### 10.1 いきものステータス基盤
`card_states` → 生き物の状態を導出する純ロジック（テスト可能）。

- [ ] 成長段階導出（`stability` → 卵/孵化/成長期/成体/完全体）
- [ ] 世話状態導出（`due` 超過度 → 健やか/空腹/しおれ/危篤/死）
- [ ] `lapses`（弱り）・`difficulty`（個性）の反映
- [ ] しきい値の実データ検証・調整（FSRS retention 依存）
- [ ] ユニットテスト（段階遷移・境界値）

### 10.2 ハビタット（庭）ビュー
デッキを生き物グリッドで表示。「今日エサが必要な子」を学習開始導線に。

- [ ] デッキ単位の生き物グリッドUI（成長段階・世話状態を表示）
- [ ] 「今日世話が必要 N匹」サマリー → ワンタップ学習開始
- [ ] オフライン（Dexie card_states）から導出
- [ ] 学習画面に「エサやり/水やり」演出・成長アニメ

### 10.3 死＆復活（墓石→お墓参り）
放置の帰結を可視化しつつ、SRSの科学を壊さない。

- [ ] 致命的長期放置で墓石化（コスメティックのみ、card_states 不変）
- [ ] 久々の復習→FSRS自然リセット（Again→stability低下）で卵から育て直し
- [ ] 墓石タップ→復習（=お墓参り）で復活する導線（永久ロスト禁止＝安全弁）
- [ ] グレイブヤードビュー（復活可能な子の一覧）

### 10.4 個別の見た目（手続き生成＋インプリント）
「全部別の絵」を現実的コストで。記憶術（視覚化）としても機能。

- [ ] 手続き生成スプライト（単語IDシード → SVGパーツ合成、自動でユニーク）
- [ ] 初回インプリント入力UI（絵文字/写真/候補から「顔」を刻む）
...
- [ ] デイリーミッション（今日◯匹の世話）+ プッシュ通知連携（Phase 12.3）
- [ ] クラスランキング（**絶対量でなく成長率**で競う／オプトアウト可）
- [ ] アチーブメントバッジ（初の完全体・100匹の庭・お墓参り復活 等）
...
**Phase**: コロケーションパイロットに文脈アシスト（日本語の場面リード文）追加・push完了。次は Phase 10
**最終更新**: 2026-06-14
**次のタスク**: デプロイ反映確認 → Phase 10「記憶のいきもの育成」着手

### 次回セッションでやること

1. **デプロイ反映確認**: 文脈アシスト（StudyCard 改修＋DB更新済み）。Vercel デプロイ後に再ログイン→パイロットデッキで表面最上部のリード文を実機確認
2. **Phase 10 着手**: 記憶のいきもの育成（@docs/memory-creatures-design.md）。まず 10.1 いきものステータス基盤（card_states → 成長/世話状態の純ロジック＋テスト）から。アート方針は「癒しモンスター系」で確定済み
3. **未決定論点を詰める**: 種の割り当て方／成長段階しきい値／インプリント入力手段／墓場UX
