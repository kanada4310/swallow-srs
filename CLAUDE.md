...
- **最終更新**: 2026-07-08
- **直近**: 学習開始のデッキ選択＋フィルタサブデッキ表示＋サブデッキ配布＋生徒別設定の4機能を実装・デプロイ。**マイグレーション 022/023 は適用済み**（Supabase SQLエディタ経由・バックフィルで継承配布行49件生成を確認）。※CLI の `db push` はアクセストークン切れで使えず、以後もマイグレーションは SQLエディタ貼り付けで適用する運用（`npx supabase login` を通せば従来どおり db push 可）
