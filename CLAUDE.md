...
- **★2026-08-27 第11便（ADR `20260827-pen-role-band-and-colors`）: 働きの高さ・階層の色・
  ...
  （0=濃い青 `#00427A` / 1=濃い橙 `#7A3800` / 2=明るい青 `#3E8ED0` / 3=明るい橙 `#B87200`。
...
- **講師限定**: `syntaxProblemsFor(base, instructor, isTeacher)`（純関数・`src/lib/reading/syntax-instructor.ts`）。
  生徒は従来の練習3問のまま＝**自動テストで固定**。正解表を読む側は `syntax-instructor-load.ts` に分け
  **講師のときだけ動的 import**（生徒の画面にデータを積まない。1ページぶん 25.6kB → 11.8kB）
